...
public class HuffmanCompressionUtilities {
	...
	void buildHuffmanTree(boolean minimize) {
		...
		while(!queue.isEmpty()) {
			...
			HuffmanTreeNode connector = new HuffmanTreeNode(sum, left, right);
			queue.add(connector);
	...
	void createHuffmanCodes(HuffmanTreeNode node, String code, int level) {
		...
		HuffmanTreeNode left = node.getLeft();
		HuffmanTreeNode right = node.getRight();
		if(left != null) {
			createHuffmanCodes(left, code+"0", level+1);
		}
		if(right != null) {
			createHuffmanCodes(right, code+"1", + level+1);
		}
	}
	...
	private int traverseTree(HuffmanTreeNode root, String binStr, int index) {
		...
		if(root.getRight() == null && root.getLeft() == null) {
			...
			if(binStr.charAt(index) == '1') {
				int t = traverseTree(root.getRight(), binStr, index+1);
				//System.out.println("t =" + t);
				return t;
			}
			else {
				int l = traverseTree(root.getLeft(), binStr, index+1);
	...
	/**
	 * Return index so can remove from bin str.
	 *
	 * @return the int to return
	 */
	public int returnIndexSoCanRemoveFromBinStr() {
		return retIndex;
	}
